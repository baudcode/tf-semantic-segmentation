...
def get_args():
    ...
    def list_type(x): return list(map(int, x.split(",")))
    ...
    parser.add_argument('-l', '--losses', default=['categorical_crossentropy'], type=list_type, choices=list(losses_by_name.keys()))
    parser.add_argument('-lm', '--metrics', default=['iou', 'dice', 'psnr'], type=list_type, choices=list(losses_by_name.keys()))
    parser.add_argument('-lw', '--loss_weights', default=[1.0, 1.0, 500.0], type=list_type)
...
def main():
    ...
    callbacks.append(kcallbacks.EarlyStopping(monitor='val_loss', mode='min', min_delta=0, patience=50, verbose=1))
    ...
    input_shape = (args.size[0], args.size[1], 3 if args.color_mode ==
    ...
    print("shapes: ", model.output.shape, model.input.shape)
    ...
    metrics = [f1_score_keras, f2_score_keras, iou_score_keras, precision_keras, recall_keras]
    ...
    if args.train_on_generator:
        ...
        train_preprocess_fn = preprocessing_ds.get_preprocess_fn(args.size, is_training=True)
        train_ds = train_ds.map(train_preprocess_fn)
        train_ds = preprocessing_ds.prepare_dataset(train_ds, args.batch_size, shuffle_buffer_size=100, buffer_size=50)

        val_ds = reader.get_dataset(DataType.VAL)
        val_preprocess_fn = preprocessing_ds.get_preprocess_fn(args.size, is_training=False)
