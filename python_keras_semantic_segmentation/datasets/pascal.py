...
class PascalVOC2012(Dataset):
    ...
    def raw(self):
        extracted = download_and_extract(self.DATA_URL, self.cache_dir)
        img_set_seg_dir = os.path.join(
            extracted, "VOCdevkit/VOC2012/ImageSets/Segmentation")
        orig_images_dir = os.path.join(
            extracted, "VOCdevkit/VOC2012/JPEGImages/")
        img_seg_dir = os.path.join(
            extracted, "VOCdevkit/VOC2012/SegmentationObject/")

        img_ann_dir = os.path.join(extracted, "VOCdevkit/VOC2012/Annotations/")

        trainset = PascalVOC2012._get_set(
            orig_images_dir, img_seg_dir, img_ann_dir, os.path.join(img_set_seg_dir, "train.txt"))

        valset = PascalVOC2012._get_set(
            orig_images_dir, img_seg_dir, img_ann_dir, os.path.join(img_set_seg_dir, "val.txt"))
