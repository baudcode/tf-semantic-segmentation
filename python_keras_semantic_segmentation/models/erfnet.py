...
def conv(x, filters, kernel_size, strides=1, norm='batch', activation='relu', l2=None, rate=1, deconv=False):
    ...
    if deconv:
        ...
        y = layers.Conv2DTranspose(filters, kernel_size, strides=strides, padding='SAME', activation=activation, dilation_rate=rate)(x)
...
def factorized_module(x, dropout=0.3, dilation=[1, 1]):
    n = K.int_shape(x)[-1]
    y = conv(x, n, [3, 1], rate=dilation[0], norm=None)
    y = conv(y, n, [1, 3], rate=dilation[0])
    y = conv(y, n, [3, 1], rate=dilation[1], norm=None)
    y = conv(y, n, [1, 3], rate=dilation[1])
...
def erfnet(input_shape=(256, 256, 1), num_classes=3):
